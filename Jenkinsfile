node {
    ...
    stage('Deploy') {
        ...
        withCredentials([string(credentialsId: 'Discord_Token', variable: 'token'), string(credentialsId: 'Discord_Owner', variable: 'owner')]) {
            sh 'docker run -d -p 8083:8080 --restart unless-stopped --name discord-bot-node -e PORT=8080 -e TOKEN=$token -e OWNER=$owner alexanderwyss/discord-bot-node:latest'
        }
    }
}
